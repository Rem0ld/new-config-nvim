...
map("n", "<leader>j", ":m .+1<cr>==", { desc = "Move down" })
map("n", "<leader>k", ":m .-2<cr>==", { desc = "Move up" })
...
if Util.has("nvim-bufferline.lua") then
  ...
  map("n", "[b", "<nop>", { desc = "Prev buffer" })
  map("n", "]b", "<nop>", { desc = "Next buffer" })
else
...
-- save file
map({ "i", "v", "n", "s" }, "<C-s>", "<nop>", { desc = "Save file" })
